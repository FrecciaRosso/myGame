(function(){
	...
	Layout.prototype.closeGameOverWindow = function() {
		document.getElementsByClassName("game-over-window")[0].style.display = "none";
		this.startNewGame();
	}
	...
	Layout.prototype.layOutCards = function(skin, level) {
		...
		var cells = document.getElementsByClassName(level)[0].getElementsByClassName("divTableCell");
		//var covers = document.getElementsByClassName(level)[0].getElementsByClassName("cover");
		var cards = document.getElementsByClassName(level)[0].getElementsByClassName("card");
		var positions = this.setCardsPositions(level);
		for (var i = 0; i < cards.length; i++) {
			cells[i].classList.remove("disable-cell");
			cards[i].style.backgroundImage = 'url("'+this.parsePathToImage(pics[positions[i]], level + '/' + skin)+'")';
	...
	document.addEventListener("DOMContentLoaded", function(event) { 
		...
		var closeGameOverWindBtn = document.getElementById("closeGameOverWind");
		closeGameOverWindBtn.addEventListener('click', function() {
			layout.closeGameOverWindow()
		} , false);	
